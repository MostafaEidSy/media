                ...
                <div class="col-sm-12">
                    <div class="iq-card">
                        ...
                        <div class="iq-card-body">
                            <form action="{{route('admin.show.season.update')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                    ...
                                    <div class="col-lg-12">
                                        <div class="row">
                                            <div class="col-md-6 form-group">
                                                <input type="text" name="title" id="title" class="form-control" placeholder="Name" required="required" value="{{old('title', $season->name)}}">
                                            ...
                                            <div class="col-md-12 form-group">
                                                <select class="selectpicker" name="video[]" multiple="multiple" data-live-search="true" title="Please Choose At Least One Video" required="required">
                                                    @foreach($videos as $video)
                                                            <option value="{{$video->id}}" @foreach($season->videos as $all) @if($all->video_id == $video->id) selected @endif @endforeach>{{$video->title}}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-12 form-group">
                                                <select class="selectpicker" name="show" data-live-search="true" title="Please Choose a Show" required="required">
                                                    @foreach($shows as $show)
                                                        <option value="{{$show->id}}" @if(old('show', $season->show_id) == $show->id) selected @endif>{{$show->title}}</option>
                                                    @endforeach
                                                </select>
                                                @error('show')<span class="small text-danger">{{$message}}</span>@enderror
                                            </div>
                                            <div class="col-md-6 form-group">
                                                <input type="text" name="duration" class="form-control" placeholder="Running Time in Minutes" required="required" value="{{old('duration', $season->duration)}}">
                                                @error('duration')<span class="small text-danger">{{$message}}</span>@enderror
                                            </div>
                                            <div class="col-md-6 form-group">
                                                <input class="form-control date-input basicFlatpickr" name="date_season" type="date" placeholder="Selete Date" required="required" value="{{old('date_season', $season->date_season)}}">
                                                @error('date_season')<span class="small text-danger">{{$message}}</span>@enderror
                                            </div>
                                            <div class="col-12 form-group">
                                                <textarea id="text" name="description" rows="5" class="form-control" placeholder="Description">{{old('description', $season->description)}}</textarea>
